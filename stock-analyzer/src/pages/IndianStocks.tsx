...
function CandlestickChart({ data, color }: { data: CandleData[], color: string }) {
  ...
  useEffect(() => {
    ...
    // Tooltip
    const tooltip = d3.select('body').selectAll('.candle-tooltip-india').data([1])
      .join('div')
      .attr('class', 'candle-tooltip-india')
...
export default function IndianStocks() {
  ...
  useEffect(() => {
    getIndianStocks().then(setStocks).catch(() => {})
  }, [])
  ...
  const handleAddToPortfolio = async () => {
    ...
    const market = selectedStock.market ?? 'india'
    ...
    <div style={{ padding: '24px' }}>
      <div style={{ marginBottom: '24px' }}>
        ...
        <p style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>
          BSE Listed Stocks — prices in INR (₹) · End-of-day data via Alpha Vantage
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: '16px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div style={{ background: 'var(--bg-card)', border: '1px solid rgba(245,197,24,0.15)', borderRadius: '8px', padding: '12px' }}>
            <div style={{ fontSize: '0.55rem', color: 'var(--text-muted)', letterSpacing: '0.15em', marginBottom: '8px' }}>SEARCH</div>
            <input type="text" placeholder="TCS, Reliance, HDFC..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} style={inputStyle} />
          </div>

          <div style={{ background: 'var(--bg-card)', border: '1px solid rgba(245,197,24,0.15)', borderRadius: '8px', overflow: 'hidden', maxHeight: '65vh', overflowY: 'auto' }}>
            <div style={{ padding: '8px 14px', background: 'rgba(245,197,24,0.05)', borderBottom: '1px solid rgba(245,197,24,0.1)', fontSize: '0.55rem', color: '#F5C518', letterSpacing: '0.2em' }}>
              BSE LISTED STOCKS
            </div>
            {filtered.map(stock => (
              <div key={stock.symbol} onClick={() => handleSelectStock(stock)}
                style={{ padding: '10px 14px', cursor: 'pointer', borderBottom: '1px solid rgba(255,255,255,0.03)', background: selectedStock?.symbol === stock.symbol ? 'rgba(245,197,24,0.08)' : 'transparent', borderLeft: selectedStock?.symbol === stock.symbol ? '3px solid #F5C518' : '3px solid transparent', transition: 'all 0.15s ease' }}
                onMouseEnter={e => { if (selectedStock?.symbol !== stock.symbol) e.currentTarget.style.background = 'rgba(245,197,24,0.03)' }}
                onMouseLeave={e => { if (selectedStock?.symbol !== stock.symbol) e.currentTarget.style.background = 'transparent' }}
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <div style={{ fontSize: '0.78rem', fontWeight: 700, color: selectedStock?.symbol === stock.symbol ? '#F5C518' : 'var(--text-primary)' }}>
                      {stock.symbol.replace('.BSE', '')}
                    </div>
                    <div style={{ fontSize: '0.58rem', color: 'var(--text-muted)', marginTop: '1px' }}>{stock.name}</div>
                  </div>
                  <div style={{ fontSize: '0.5rem', color: 'rgba(245,197,24,0.4)', letterSpacing: '0.1em' }}>BSE</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {!selectedStock ? (
            <div style={{ background: 'var(--bg-card)', border: '1px solid rgba(245,197,24,0.1)', borderRadius: '8px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '400px' }}>
            ...
            <>
              {/* Quote Header */}
              <div style={{ background: 'var(--bg-card)', border: '1px solid rgba(245,197,24,0.2)', borderRadius: '8px', padding: '20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      <span style={{ fontFamily: 'Syne, sans-serif', fontSize: '1.6rem', fontWeight: 800, color: '#F5C518', textShadow: '0 0 30px rgba(245,197,24,0.4)' }}>
                        {selectedStock.symbol.replace('.BSE', '')}
                      </span>
                      <span style={{ fontSize: '0.58rem', color: 'rgba(255,255,255,0.3)', background: 'rgba(255,255,255,0.05)', padding: '3px 8px', borderRadius: '4px', letterSpacing: '0.1em' }}>BSE</span>
                    </div>
                  ...
                  {loadingQuote ? (
                    ...
                    <div style={{ textAlign: 'right' }}>
                      ...
                      <div style={{ fontSize: '0.85rem', fontWeight: 600, color: quote.changePercent >= 0 ? '#4CAF50' : '#F44336', marginTop: '4px' }}>
                        ...
                        <span style={{ fontSize: '0.65rem', color: 'var(--text-muted)', marginLeft: '8px' }}>prev close</span>
                      </div>
                    </div>
                  ) : null}
                </div>
              ...
              {/* Candlestick Chart */}
              <div style={{ background: 'var(--bg-card)', border: '1px solid rgba(245,197,24,0.15)', borderRadius: '8px', padding: '20px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '6px' }}>
                    {['1mo', '3mo', '6mo'].map(p => (
                      <button key={p} onClick={() => handlePeriodChange(p)}
                        style={{ padding: '4px 10px', background: period === p ? 'rgba(245,197,24,0.15)' : 'transparent', border: `1px solid ${period === p ? '#F5C518' : 'rgba(255,255,255,0.1)'}`, borderRadius: '4px', color: period === p ? '#F5C518' : 'var(--text-muted)', fontSize: '0.6rem', cursor: 'pointer', fontFamily: 'JetBrains Mono, monospace', letterSpacing: '0.1em', transition: 'all 0.15s ease' }}>
                        {p.toUpperCase()}
                      </button>
                    ))}
                  </div>
                ...
                {loadingChart ? (
                  <div style={{ height: '340px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: '0.7rem', color: '#F5C518', letterSpacing: '0.2em', marginBottom: '8px' }}>LOADING CHART DATA</div>
                      <div style={{ fontSize: '0.6rem', color: 'var(--text-muted)' }}>Fetching from Alpha Vantage...</div>
                    </div>
                  </div>
                ) : candleData.length > 0 ? (
              ...
              <div style={{ background: 'var(--bg-card)', border: '1px solid rgba(245,197,24,0.15)', borderRadius: '8px', padding: '20px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: '12px', alignItems: 'end' }}>
                  <div>
                    ...
                    <input type="number" placeholder="e.g. 10" value={quantity} onChange={e => setQuantity(e.target.value)} style={inputStyle} />
                  </div>
                  <div>
                    <div style={{ fontSize: '0.55rem', color: 'var(--text-muted)', letterSpacing: '0.15em', marginBottom: '6px' }}>BUY PRICE (₹ INR)</div>
                  ...
                  <button onClick={handleAddToPortfolio}
                    style={{ padding: '9px 18px', background: 'rgba(245,197,24,0.1)', border: '1px solid rgba(245,197,24,0.4)', borderRadius: '6px', color: '#F5C518', fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.15em', cursor: 'pointer', fontFamily: 'JetBrains Mono, monospace', whiteSpace: 'nowrap', transition: 'all 0.2s ease' }}
                ...
                {quantity && buyPrice && parseFloat(quantity) > 0 && parseFloat(buyPrice) > 0 && (
                  <div style={{ marginTop: '10px', fontSize: '0.65rem', color: 'var(--text-muted)', padding: '6px 10px', background: 'rgba(0,0,0,0.2)', borderRadius: '4px' }}>
                    Total Cost: <span style={{ color: '#F5C518', fontWeight: 700 }}>₹{(parseFloat(quantity) * parseFloat(buyPrice)).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                  </div>
                )}
